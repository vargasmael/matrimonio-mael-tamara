...
def main():
    ...
    lines_tg = []
    lines_tg.append(f"📋 *RSVP Mael & Tamara · nuevos hoy*")
    lines_tg.append("")
    ...
    # --- Markdown para WhatsApp (versión corta, una sola línea por invitado) ---
    lines_wa = []
    lines_wa.append(f"*RSVP Mael & Tamara — nuevos hoy*")
    lines_wa.append("")
    if len(new) == 1:
        lines_wa.append(f"✅ {new[0][0]} confirmó.")
    else:
        names = ", ".join(n for n, _ in new)
        lines_wa.append(f"✅ {len(new)} nuevos confirmados: {names}.")
    lines_wa.append(f"Total: {total_attending}/109.")
